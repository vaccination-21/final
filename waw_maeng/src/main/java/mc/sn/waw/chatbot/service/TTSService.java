package mc.sn.waw.chatbot.service;
	...
	public String chatbotTextToSpeech(String message) {
		 String clientId = "dhmge0vn1o";//애플리케이션 클라이언트 아이디값";
	        ...
	        try {
	            ...
	            con.setDoOutput(true);
	            DataOutputStream wr = new DataOutputStream(con.getOutputStream()); // 아웃풋 스트림 열기
	            wr.writeBytes(postParams);
	            wr.flush();
	            wr.close();
	            int responseCode = con.getResponseCode(); //서버 가서 잘 갔는지 확인
	            BufferedReader br;
	            if(responseCode==200) { // 정상 호출
	                InputStream is = con.getInputStream();//인풋 스트림 열기
	                int read = 0;
	                byte[] bytes = new byte[1024];
	                // 랜덤한 이름으로 mp3 파일 생성
	                String tempname = Long.valueOf(new Date().getTime()).toString();
	                ...
	                OutputStream outputStream = new FileOutputStream(f); //outputStream을 f에 설정
	                while ((read =is.read(bytes)) != -1) { //음성이 01010101001이렇게 표현되니까??
	                    outputStream.write(bytes, 0, read); //서버 갔다가 돌아온 데이터 받기
	                }
	                is.close();
	            } else {  // 오류 발생
	                br = new BufferedReader(new InputStreamReader(con.getErrorStream()));
